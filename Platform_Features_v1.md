...
## 0. РЕАЛІЗОВАНІ ФУНКЦІЇ v1.0 (Production Ready)

### API Endpoints

**Edge Management** (Gap #7) — управління зв'язками між нодами:
- `POST /api/funnels/:botId/edges` — Створити новий edge
- `PUT /api/funnels/:botId/edges` — Замінити всі edges разом
- `DELETE /api/funnels/:botId/edges/:edgeId` — Видалити конкретний edge

**Node Statistics** (Gap #9) — моніторинг продуктивності нод:
- `GET /api/funnels/:botId/nodes/:nodeId/stats` — Статистика ноди (error rate, indicator)

**Prerequisites Check** (Gap #2) — валідація перед стартом:
- `POST /api/funnels/:botId/check-prerequisites` — Перевірка необхідних файлів/налаштувань

**Context Passing** (Gap #4) — завантаження контексту:
- `GET /api/sessions/:sessionId/context` — Завантажити попередні файли як контекстні змінні

### Нові Node Types

Розширено палітру нод у редакторі з новими типами:

1. **LoadFile** (📂) — завантаження файлів з сховища
   - Виберіть тип файлу (cashflow_articles, pl_articles, business_process, тощо)
   - Вибір дії при відсутності (skip/error/default)
   - Зберігає в context як `context.[fileName]`

2. **HttpRequest** (🌐) — GET/POST запити до API
   - URL з поддержкою змінних контексту
   - Методи: GET, POST, PUT, DELETE
   - Body як JSON з контекстом ({{variable}} інтерполяція)
   - Результат → `context.apiResponse`

3. **Tag** (🏷️) — теґування підписників
   - Action: add/remove/check
   - Назва тегу з контекстом
   - Для сегментації аудиторії в CRM

4. **ABTest** (🧪) — розподіл на варіанти
   - Налаштувати процентні розподіли (50/50, 70/30, тощо)
   - Кожна гілка йде окремо за умовою
   - Запис варіанта в slot для аналізу

5. **Wait (Enhanced)** (⏳) — гнучка пауза
   - Тривалість в хвилинах/годинах
   - Дні тижня (Пн-Пт / тільки вихідні)
   - Тиха зона (не писати з 22:00 до 8:00)
   - Перевірка часового поясу підписника

### UI Pages

**Logs** (📡 вкладка в меню) — Об'єднаний журнал логів
- Фільтрація по табах: Всі, Claude, Telegram, Google Sheets, Помилки
- Пошук по повідомленню, сервісу, botId
- Розширення для перегляду request/response/stack trace
- Рівні логування: error (червоний), warn (жовтий), info (синій)

**Settings** (⚙️ вкладка в меню) — Конфігурація платформи
- **MCP Integration** — URL для підключення Claude до платформи з інструкціями
- **Global Keys** — управління глобальними ключами проекту (Apps Script URL, Telegram Token, Instagram Token)
- **Webhook** — адреса вебхука з поясненням динамічної маршрутизації (/webhook/telegram/{botSlug})
- **Account** — профіль адміна (email, роль, API токен, пароль, logout)

### Navigation Restructuring

Реорганізована бічна панель навігації:

| Нова структура | Попередня | Зміни |
|---|---|---|
| Воронки | Воронки | Без змін |
| Проєкти | Проєкти | Без змін |
| Дашборд | Дашборд | Без змін |
| 👥 Підписники | Users | Перейменовано |
| 📡 Логи | API логи + Помилки | Об'єднано в один розділ |
| ⚙️ Налаштування | [Новий] | Додано новий розділ |

### Backend Improvements

- Валідація схем Zod для всіх новий endpoints
- Обробка помилок через `NotFoundError` та стандартизовані відповіді
- Асинхронна обробка через `asyncHandler` middleware
- Підтримка пошуку контексту користувача з сортуванням по свіжості файлів
