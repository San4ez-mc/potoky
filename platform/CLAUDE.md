...
### 15.11 Об'єкти в messagesTemplate ламають JSON — Claude отримує «Продовжуємо діалог»
...
### 15.13 message-нода перед agent-нодою в dialogMode = два привітання

**Помилка (2026-09-08):** граф `start → js → msg_intro → agent_main`. `msg_intro` слав
привітання, а `agent_main` у `dialogMode` без вхідного повідомлення відпрацьовував
`startTrigger` і вітався ще раз. Клієнтка отримувала два майже однакові тексти підряд.

**Чому це не лікується промптом:** агент-нода в `dialogMode` говорить першою за
означенням — це не збій, а її робота. Скільки не пиши «не вітайся вдруге», перше
повідомлення все одно згенерують ДВА вузли.

**Правило:** перший хід озвучує РІВНО ОДИН вузол. Якщо після старту стоїть
`agent`-нода в `dialogMode` — message-ноди перед нею бути не повинно; потрібний
детермінований текст передається їй через `startTrigger` (він рендериться як
шаблон, тож `{{context.*}}` там працює).

### 15.14 Prisma на проді: блокові коментарі й `db push` (2026-09-25)

**Дві помилки за один деплой нагадувань:**
1. `/** ... */` у `schema.prisma` — **синтаксична помилка Prisma**. Коментарі тільки `//` і `///`.
2. `npx prisma db push` звіряє **всю** схему і падає на `permission denied for table global_keys`
   (чужа таблиця, інший власник) — тобто ним не можна додати навіть свою нову таблицю.
   **Правило:** нову таблицю на проді додавати через `npx prisma db execute --file` з явним
   `CREATE TABLE IF NOT EXISTS` (+ індекси), а `prisma generate` — окремо. Схему в репо все одно
   оновлюємо: вона джерело правди для клієнта й наступних середовищ.
...
## 17. Каталог нод і стандарт воронок — ЧИТАТИ ПЕРЕД БУДЬ-ЯКОЮ ВОРОНКОЮ

**Перед тим як додавати/змінювати ноду — відкрий `.claude/skills/fineko-funnel-standard/SKILL.md`.**
Там:
- **§1 Маршрутизатор** — «треба X → бери ноду Y» (швидкий вибір).
- **§2 Каталог нод** — усі типи з реальними полями `data.*`, які читає двигун, і поясненням коли брати / коли НЕ брати.
- **§3 Архітектурні правила потоку** — одна нода = одна дія, немає глухих кутів, кожне повідомлення веде далі.
- **§4 Антипатерни** — реальні помилки з продакшену (демо-фолбеки, гілки в нікуди, осиротілі ноди, `json_output` з вигаданими значеннями, `notifyAdmin` не туди тощо).
- **§5 Тестування** — ≥10 покупців із різною поведінкою + як діагностувати через вкладку «Ноди».
- **§6 Чекліст готовності**.

**Коротко найважливіше:**
- Сповіщення менеджеру — **`notifyTg`** (chat_id з ключа воронки), НЕ `notifyAdmin` (має системні фолбеки → «bot can't initiate conversation»).
- Instagram: **немає inline-кнопок** (посилання текстом), але **є альбом до 10 фото** в одному повідомленні.
- Зовнішні мутації (замовлення постачальнику, платіж) — завжди під `*_DRY_RUN` + гард `context.testMode`.
- Демо/тестові дані як фолбек у проді — заборонені: краще «не визначив → покликати людину».
- Постійні зміни воронки — у **patch-файл** (джерело істини), не лише разовою міграцією.

---

## 18. Система тестування воронок (FunnelTest / TestRun) — 2026-09-23

До цього кнопка «Тест» просто ганяла 3 рандомних повідомлення (`regressionRunner.js`) і
завжди звітувала «успіх», незалежно від того, що реально відповів бот. Тепер це
повноцінна система: попап у редакторі воронки (`FunnelTestModal.jsx`) спочатку показує
список **збережених** тестів — і лише там можна створити новий / відредагувати / скопіювати
/ запустити один чи всі одразу. Нічого не запускається автоматично при відкритті попапу.

### Модель (`packages/db/schema.prisma`)
- `FunnelTest` — `steps[]` (кроки клієнта: `text`/`photo`/`forward_post`/`ad_reply`, кожен
  несе те саме, що реальний вхідний — `imageUrl`, `sharedPost{kind,mediaId,caption,url}`,
  `referral{ref,adId,source,type,adTitle}`), `expectedOutcome` (вільний текст — САМЕ по
  ньому LLM-суддя вирішує passed/failed), `connectorId` (AI-конектор-суддя, null = системний
  Claude ключ).
- `TestRun` — історія прогонів, `verdict` (JSON: `{passed, reasoning, failingNodeId,
  nodeVerdicts[]}`).
- `Message.flagged/flagNote/flaggedAt` — ручна позначка «ця відповідь неправильна» прямо в
  перегляді сесії (кнопка ⚠ на бульбашці).
- `Node.data.qaExpectation` (НЕ окрема модель, поле в JSON вузла флоу, редагується в
  `NodeEditor.jsx` — блок «Label + Description», спільний для всіх типів нод) — що ця
  конкретна нода має робити правильно. Перевіряється суддею на **КОЖНОМУ** прогоні, що
  проходить через цю ноду, незалежно від того, для чого писався сам тест — так ловляться
  регресії в нодах, не повʼязаних з конкретним тестом.

### Раннер (`apps/api/src/services/funnelTests.js`)
1. `startTestSession` з `contextOverride.noPrerun=true`, якщо перший крок НЕ текстовий
   (фото/пересланий пост/реклама) — щоб воронка стартувала рівно на цьому вхідному, як у
   живого first-contact клієнта; для текстового першого кроку — звичайний преран (бот
   вітається першим, як зазвичай).
2. Кожен крок іде через **новий** `testSession.sendTestTurn` (не старий `sendTestMessage` —
   той приймає лише голий рядок). `sendTestTurn` пише в `Message.metadata` повний конверт
   (imageUrl/sharedPost/referral) і, якщо є sharedPost/referral, патчить
   `session.context.{sharedPost,lastReferral,entryAdId}` ПЕРЕД `executeFlowStep` — той самий
   ефект, що дають `instagramHandler.extractReferral`/zernioHandler для живого вхідного.
3. Після всіх кроків — LLM-суддя (обраний `connectorId` або системний Claude ключ, модель
   `claude-sonnet-4-6`) отримує повну транскрипцію + список нод із власним `qaExpectation`,
   які реально відвідались у цьому прогоні, і повертає JSON-вердикт.
4. При провалі — `AppError` (`errorType:'test_failed'`) на кожну «винну» ноду. **Важливо:**
   `createdAt` цього `AppError` явно виставляється на момент виконання самої ноди (з
   `nodeTraces[].tsIso`), а не «зараз» — бо `NodeTraceTab` (SessionDetail.jsx) прив'язує
   помилку до картки ноди за ЧАСОВИМ ВІКНОМ, і суддя оцінює вже ПІСЛЯ завершення всього
   прогону.

### MCP (`apps/mcp/src/tools-flows.js`)
`list_tests/get_test/create_test/update_test/delete_test/duplicate_test/run_test/
run_all_tests` — тонкі обгортки над `funnelTests.js`, підключеним відносним `require`
з `apps/mcp/src/` в `apps/api/src/services/` (той самий підхід, що вже застосовують
`mcp-flows.js`/`mcp-flows-edit.js` у зворотному напрямку — жодного HTTP між процесами).

### Де запустити «всі тести одночасно» (3 місця)
1. Сам попап тестів (кнопка «▶▶ Запустити всі тести» над списком).
2. Іконка ▶ в списку воронок (`Bots.jsx`) — прямо з `/funnels`, без входу в редактор.
3. MCP tool `run_all_tests`.

### Одноклікове створення тесту з реальної сесії
У перегляді сесії (`SessionDetail.jsx`) на будь-якій бульбашці — кнопка ⚠ «Позначити
помилковим» (+ примітка) → кнопка «🧪 Створити тест з цієї сесії» → бекенд
(`funnelTests.createTestFromSession`, ендпоінт `POST /api/sessions/:id/create-test`)
бере ВСІ `user`-повідомлення сесії до позначеного включно й перетворює на `steps[]` за їхнім
реальним `metadata` (фото/пересланий пост/реферал зберігаються як є — нічого не
спрощується). Далі — редірект на `/funnel/:botId?openTest=:testId`, попап відкривається
одразу у формі редагування нового тесту.
