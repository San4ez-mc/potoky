'use strict';
/**
 * catalogHint.js — ported from the DB-stored flow node n_catalog_hint (goverla_shop CRM
 * version). Used when the customer hasn't named/shown a specific product yet: narrows the
 * catalog by category word / color word / just-shown-list follow-up, and returns either a
 * short numbered list (catalogHint) or a single confident pick (catalogHintPick) for the
 * caller to re-run matchProduct() against.
 *
 * n_catalog_hint_prep (the sibling flow node that used to run before this one) computed
 * catalogHintWants/catalogHintColorWords/catalogHintStem as an optimization to avoid an
 * unconditional CRM fetch on a separate flow-graph node — but this function already
 * recomputes all of that itself from scratch and never consumed prep's output (verified by
