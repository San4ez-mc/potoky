 ...
 * dated comment below documents a real historical incident fix and must not be dropped
 * during future edits. What changed vs. the original flow-node:
 *   - No longer decides on its own whether to run (early-exit guard removed) — the caller
 *     (resolveShoppingIntent.js) decides that via signal.js's classifySignal().
 *   - Runs as a plain async function (context, keys, input) instead of inside runNodeCode's
 *     new Function(...) sandbox — fetch/Buffer/console/crypto are Node globals here, same
 *     as they were injected there.
 * Returns the exact same result shape the flow node returned (dialogState, product{...},
...
// early-return guard removed 2026-09-22: gating ("should we even look") now lives in resolveShoppingIntent.js/signal.js — this module always attempts a real match when called.
...
try {
  ...
  // 2026-09-21 (живий кейс, Яна Рубан: "яка ціна костюма?" + фото костюма → реклама привʼязана
  // до A0187 (кофта), категорія не збіглась → коректно відхилено, показано список костюмів.
  // НАСТУПНЕ повідомлення "можна розмірну сітку?" не містить слова категорії — __uStem порожній
  // — перевірка невідповідності нижче просто НЕ запускається (умова на неї — !__uStem виключає),
  // тож Пріоритет 0 сліпо повторно бере ТУ САМУ вже відхилену прив'язку без жодної звірки. Раз
  ...
  // якщо сет і хоч один компонент має власну сітку — інша нота, двигун (testSession.js)
  // вже вміє зібрати сітку по кожній позиції окремо.
  if (found.isSet && setItems.some(function (x) { return x.sizeChartData && Array.isArray(x.sizeChartData.sizes) && x.sizeChartData.sizes.length; })) {
    __sizeChartNote = 'Розмірна сітка є ОКРЕМО для кожної позиції комплекту — якщо клієнт попросить сітку, скажи що зараз покажеш (система сама збере таблицю по всіх позиціях).';
  }
  ...
  if (!__sizeChartUrl && !__sizeChartData && __isHeightWeight) {
    ...
    // 2026-09-13 (аудит 27-товарного тесту, тікет 6853e998): "Ми вже добре знаємо посадку цієї
    // моделі" звучало як конкретне знання САМЕ цієї моделі, хоча це загальний фолбек-текст для
    // будь-якого товару без сітки — прибрано фразу, що перебільшує впевненість, лишили чесний
    // акцент на точності підбору за зростом/вагою як методі (без твердження "ми знаємо цю модель").
  ...
  // 2026-09-12 (власник, живий кейс Максим Заводько): голе "Цікавить? 😊" після презентації
  // КОМПЛЕКТУ не дає клієнту зрозуміти, що можна взяти лише окремі позиції — заміна на конкретне
  // питання про весь набір чи окремі товари.
  var __followUpQuestion = __isClothing
    ? ('👉 Підкажіть, будь ласка, ' + (__paramAsk || 'зріст і вагу') + ' — підберу ідеальний розмір 😊')
    : (found.isSet ? 'Підкажіть, будь ласка, вас цікавить весь комплект, чи окремі товари з нього? 😊' : 'Цікавить? 😊');
