...
async function executeFlowStep({ sessionId, incomingUserMessage = null, incomingFile = null, incomingImageUrl = null }) {
    ...
    // Магазинні перехоплювачі — тільки для воронок, які їх свідомо ввімкнули.
    //
    // Детектори нижче («хочу живу людину», «повернення товару») писались під
    // магазини, але жили у СПІЛЬНОМУ двигуні й спрацьовували в кожній воронці.
    // Асистентка рекрутингової агенції на розповідь клієнтки про компанію
    // відповіла «зараз покличу менеджера» і замовкла: слово зі щоденної лексики
    // рекрутингу збіглося зі стоп-словом магазину. Гірше за саме повідомлення те,
    // що хід перехоплювався ДО агента — розшифроване голосове до нього не доїхало.
    //
    // Вмикається ключем воронки SHOP_HANDOFF=1. Не «здогадатись по наявності
    // ADMIN_TELEGRAM_ID», а явно: воронки різних клієнтів не повинні перетинатись
    // через побічний ефект спільного коду.
    let _handoffPossible = false;
    try {
        const _hoRow = await db.funnelKey.findFirst({
            where: { botId: session.botId, key: 'SHOP_HANDOFF' },
            select: { value: true },
        });
        _handoffPossible = /^(1|true|on|yes)$/i.test((_hoRow?.value || '').trim());
        ...
        if (node.type === 'claude') {
            ...
            if (mode === 'dialog') {
                ...
                if (exit.parsed && exit.parsed.wantsSizeChart === true && !exit.parsed.askManager) {
                    ...
                    const _scText = String((ctx.product && ctx.product.sizeChartText) || '').trim();
                    if (_scAlready) {
