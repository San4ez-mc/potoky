...
for(var ari=0; ari<articleCandidates.length && !itemId; ari++){
  ...
  for(var ci=0; ci<cats.length && !itemId; ci++){
    ...
    var hit=null;
    // Аудит 2026-09-04: якщо клієнт має конкретний розмір, а його нема — НЕ беремо "будь-який
    // наявний" (раніше клієнту їхав інший розмір). Помилка + supplierNeedsManual.
    if(wantSize){
      hit=opts.filter(function(o){ return o.avail && String(o.label).replace(/[^0-9A-Za-zXL]/g,'')===wantSize; })[0];
      if(!hit){ var __availLabels=opts.filter(function(o){return o.avail;}).map(function(o){return o.label||o.id;}).join(', '); return { supplierOrderResult:'❌ easydrop: розміру '+wantSize+' для «'+article+'» немає в наявності у постачальника (є: '+(__availLabels||'—')+')', supplierOrderStatus:'error', supplierNeedsManual:true }; }
    } else { hit=opts.filter(function(o){ return o.avail; })[0]; }
...
// Аудит 2026-09-04: лише збіг за назвою (без "будь-яке м." і без фолбеку на перший елемент списку — це давало чужі міста).
function __cityNorm(x){ return String(x||'').toLowerCase().replace(/^(м|с|смт|сел|селище)\.?\s+/i,'').replace(/[’'`]/g,'').trim(); }
var cityPick=cityOpts.filter(function(x){ var n=__cityNorm(String(x).split(',')[0]); return n===__cityNorm(cityQ) || n.indexOf(__cityNorm(cityQ))===0; })[0]||'';
...
var whPick=bnum2?(whOpts.filter(function(x){ return new RegExp('№\\s*'+bnum2+'(?!\\d)').test(String(x)); })[0]||''):'';
if(!bnum2) return { supplierOrderResult:summary+'\n❌ easydrop: не вказано номер відділення/поштомата', supplierOrderStatus:'error', supplierNeedsManual:true };
if(!cityPick||!whPick) return { supplierOrderResult:summary+'\n❌ easydrop: не підібрав місто/відділення в автокомпліті (місто «'+cityQ+'», відділення №'+bnum2+') — без фолбеку на перший варіант', supplierOrderStatus:'error', supplierNeedsManual:true };
