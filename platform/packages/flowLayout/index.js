...
// 10. Периферія для службових hub-нод (аудит 2026-08-26, ідея з Gemini): notifyTg/
//     notifyAdmin з кількома вхідними ребрами з різних гілок — на правий край рядка,
//     а не в середину (барицентр інакше тягне їх у центр графа, роблячи павутину).
//
...
function computeAutoLayout(nodesIn, edgesIn) {
    ...
    // ── Периферія для службових hub-нод (правило 10, ідея з аудиту 26.08 — Gemini) ──
    // notifyTg/notifyAdmin ("сигнал адміну", "передача на менеджера") часто мають
    // вхідні ребра з геть різних гілок воронки — барицентр тягне їх у середину графа,
    // і саме вони стають вузлом "павутини". Якщо в такої ноди >1 вхідних ребра —
    // виносимо її на правий край ЇЇ рядка замість того, щоб усереднювати позицію.
    const PERIPHERAL_TYPES = new Set(['notifyTg', 'notifyAdmin']);
    nodes.forEach((n) => {
        if (!PERIPHERAL_TYPES.has(n.type) || inEdges[n.id].length <= 1) return;
        const sameRowCols = nodes.filter((m) => row[m.id] === row[n.id] && m.id !== n.id).map((m) => col[m.id]);
        const maxCol = sameRowCols.length ? Math.max(...sameRowCols) : col[n.id];
        col[n.id] = maxCol + 1;
    });
    decollide();
