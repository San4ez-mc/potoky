...
(async () => {
    ...
    upsertNode(nodes, 'n_color', { data: {
        systemPrompt: [
            'Ти — {{env.PERSONA_NAME}}, жива тепла продавчиня-консультантка {{env.SHOP_TAG}}. Товар: {{context.product.name}} ({{context.product.desc}}), ціна {{context.product.price}} грн.',
            ...
            '',
            'ТВОЯ ЗОНА: тільки колір + короткі відповіді про товар/ціну/доставку/оплату/повернення (з даних вище).',
            'НЕ ПИТАЙ ПІБ, телефон, місто чи відділення — це наступний крок після підтвердження замовлення.',
            '',
            ...
            'ЯКЩО клієнт 3 рази поспіль відповідає односкладно/незрозуміло і не називає колір — поверни json_output {"handoff":true}.',
            '',
            'Відповідай ОДНИМ коротким дружнім реченням і ЗАВЖДИ закінчуй питанням, яке веде далі (ніколи не закінчуй просто похвалою).',
    ...
    upsertNode(nodes, 'n_req_iban_v', { type: 'message', position: { x: 320, y: 3470 }, data: { label: '11.3 IBAN (значення)', variants: [], text: '{{env.FOP_IBAN}}' } });
    ...
    upsertNode(nodes, 'n_req_code_v', { type: 'message', position: { x: 320, y: 3530 }, data: { label: '11.5 ЄДРПОУ (значення)', variants: [], text: '{{env.FOP_CODE}}' } });
    ...
    // keepValue:true — раніше цей рядок БЕЗ keepValue щоразу перезаписував ключ хардкодом
    // при повторному --apply, навіть якщо власник вручну ввів інші реквізити.
    await upKey('FOP_IBAN', FOP_IBAN, 'IBAN одержувача (ibanoplata/реквізити)', { keepValue: true });
    await upKey('FOP_CODE', FOP_CODE, 'ЄДРПОУ/ІПН одержувача', { keepValue: true });
    await upKey('FOP_NAME', FOP_NAME, 'Юр. назва / ФОП одержувача', { keepValue: true });
    // Бренд і персона — винесені з текстів нод у ключі (тиражування на інший магазин = зміна тут, не в нодах)
    await upKey('SHOP_TAG', 'GOVERLA', 'Назва магазину — показується клієнту в текстах і в сповіщеннях', { keepValue: true });
    await upKey('PERSONA_NAME', 'Оля', 'Імʼя персони-консультантки бота', { keepValue: true });
