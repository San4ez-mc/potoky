...
const SIZE_PROMPT_V3 = '\nДОДАТКОВО: (а) зріст у метрах («1,78», «1.78 м») = 178 см — переводь сам; (б) якщо клієнт разом із параметрами назвав КОЛІР зі списку кольорів товару («чорний 182/100», «S в графітному») — додай у той самий json_output поле "color":"<колір як у списку>", щоб не перепитувати; (в) якщо клієнт надіслав фото (текст "[фото]") — по фото розмір не визначаю, скажи це одним реченням і попроси зріст і вагу (товар НЕ змінюй); (г) якщо клієнт назвав власні заміри (плечі/рукав/ширина) — подякуй і скажи, що підбір іде за зростом і вагою, попроси їх; (д) якщо клієнт назвав ОБХВАТ ТАЛІЇ або згадав живіт/животик («талія 113», «є животик») — додай у json_output "waist":<см або 0>,"belly":true — система врахує це при підборі розміру, не ігноруй; (е) ТАБЛИЦЯ ЗАМІРІВ ЦЬОГО ТОВАРУ (порожньо = нема): {{context.product.sizeChartText}} — на питання про довжину виробу/рукава чи обхват грудей відповідай із неї цифрами.';
...
function applyV3(nodes, edges, notes) {
    ...
    if (has('n_size')) { const n = byId().n_size; n.data.waitAfterPresentationUnless = SIZE_FIRST_MSG_RE; n.data.keepProductOnImage = true; if (!String(n.data.systemPrompt || '').includes('ОБХВАТ ТАЛІЇ')) { n.data.systemPrompt = String(n.data.systemPrompt || '').replace(/\nДОДАТКОВО: \(а\) зріст у метрах[^\n]*/, '') + SIZE_PROMPT_V3; notes.push('size prompt v3.1 n_size'); } }
