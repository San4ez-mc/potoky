...
function applyV3(nodes, edges, notes) {
    ...
    if (has('n_size')) { const n = byId().n_size; n.data.waitAfterPresentationUnless = SIZE_FIRST_MSG_RE; n.data.keepProductOnImage = true; if (!String(n.data.systemPrompt || '').includes('[v3.2]')) { n.data.systemPrompt = String(n.data.systemPrompt || '').replace(/\nДОДАТКОВО: \(а\) зріст у метрах[^\n]*/, '') + SIZE_PROMPT_V3; notes.push('size prompt v3.2 n_size'); } }
