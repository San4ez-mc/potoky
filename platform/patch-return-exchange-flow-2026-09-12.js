...
async function patchBot(botId) {
  ...
  for (const n of NEW_NODES) {
    if (!nodeIds.has(n.id)) { outNodes.push(Object.assign({ position: { x: 0, y: 0 } }, n)); changed = true; console.log(botId, '+ node', n.id); }
    else console.log(botId, '= node вже є', n.id, '(пропускаю)');
  }
