...
async function scenarioReturnAndUnknown(name, botId) {
    ...
    ok(name, 'без товару — просить пост/артикул, без "перепрошуємо за очікування"', /пост|рілс|артикул/i.test(r.all) && !/Перепрошуємо за очікування/.test(r.all), r.all.slice(0, 160));
    r = await say(s, 'Яка доставка?');
    ok(name, 'друге повідомлення без товару — знову просить товар, без "перепрошуємо"', !/Перепрошуємо за очікування/.test(r.all), r.all.slice(0, 160));
