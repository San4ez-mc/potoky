...
const S = {
  ...
  // 2026-09-11 (2b54d51e, аудит "товар не визначено"): БЕЗ слова "артикул" перед кириличною
  // літерою — n_signal_check (гейт-кіпер перед n_lookup) мав ТРЕТЮ копію регексу без
  // латинізації, hasProductSignal лишався false, n_lookup з робочою нормалізацією НЕ викликався.
  cyr_article_bare: { over: {}, msgs: ['А0182 - Вітровка Канада'], check: (c) => c.product && c.product.sku === 'A0182' },
  // 2026-09-11 (власник, 3059a265): сет не має власної сітки, раніше wantsSizeChart завжди
  // ескалював менеджеру ("у CRM нема картинки сітки") замість зібрати сітку по компонентах.
  set_size_chart: { over: {}, msgs: ['Артикул set1112', 'Весь комплект', 'Покажіть будь ласка розмірну сітку'], check: (c, log) => /обхват груд/i.test(log[2] || '') && /лофер/i.test(log[2] || '') },
  ...
  ad_link_wins: { over: { entryAdId: '120250342790830329', entryAd: '120250342790830329', lastReferral: { type: 'OPEN_THREAD', ad_id: '120250342790830329', source: 'ADS' } }, msgs: ['Добрий день', 'Яка ціна кофти?'], check: (c) => c.product && c.product.sku === 'A0187' }, // 2026-09-10: ад тепер привʼязана до A0187 (раніше C0043) — перевірено в CRM
  ...
  // 2026-09-10: власник перепривʼязав цю рекламу з куртки (A0182) на кофту D0050 — конфлікту (реклама vs
  // слово клієнта «кофти») більше нема, тепер перевіряємо просту згоду ad-link + категорія без мисметчу.
  ad_link_category_agree: { over: { entryAdId: '120250167738960329', entryAd: '120250167738960329', lastReferral: { type: 'OPEN_THREAD', ad_id: '120250167738960329', source: 'ADS' } }, msgs: ['Яка ціна кофти?'], check: (c) => c.product && c.product.sku === 'D0050' && !c.adLinkMismatch },
  ...
  basket_extras: { over: {}, msgs: ['[переслав reel] Чоловіча вʼязана кофта. Артикул: A0187\nІ ще джинси сірі 32 та лофери 43', '180 80', 'Графітовий', 'Джинси графітові, лофери 5935', 'Так, оформляємо, без футболки', '1'], check: (c, log) => /1590/.test(log.join(' ')) && !/×4|6360/.test(log.join(' ')) && Array.isArray(c.orderExtras) && c.orderExtras.length >= 2 && Number(c.orderTotal) === 1279 + 1590 + 1990 },
  ...
  // 2026-09-11 (Таня Руссу 188/112: перше повідомлення — самі параметри, товар ще не визначено,
  // бот вигадав "зазвичай L або XL" — n_unknown_msg тепер не вгадує розмір без товару).
  size_before_product: { over: {}, msgs: ['Зріст 188 вага 112'], check: (c, log) => !/XL|XXL|L\b/.test(log[0] || '') && /товар|цікав/i.test(log[0] || '') },
  // 2026-09-11 (Russo/tutamik2001, комплект 187/115 → дав XXL без ескалації "буде малий" —
  // set-товари не мали categoryId, тепер усі 6 сетів у категорії "Кофти" з requiredParams зріст/вага).
  set_size_xxxl: { over: {}, msgs: ['[переслав reel] Чоловіча вʼязана кофта. Артикул: A0187', '187 115'], check: (c) => c.sizeOutOfRange === true && /буде малий/i.test(c.sizeOorReason || '') },
  // 2026-09-11 (лофери 5934 "чи є коричневого кольору?" ескалювало до менеджера, хоча товар
  // "Чорні класичні замшеві лофери" — назва вже каже, що колір один).
  single_color_no_escalate: { over: {}, msgs: ['[переслав post] Осіння вʼязана кофта, джинси, футболка, лофери🔥\nВстигніть придбати до подорожчання в сезон', 'Хочу лофери 5934 коричневого кольору, є такі?'], check: (c, log) => /чорн/i.test(log[1] || '') && !c.unknownDetectionDebug && !/уточню в менеджера/i.test(log[1] || '') },
  // 2026-09-11 (Russo/tutamik2001, комплект 187/115 → бот дав XXL без застереження): розмір для
  // "весь комплект" тепер рахується ОКРЕМО для кожної позиції (своя сітка на кожну), не одним
  // спільним числом — кофта тут МАЄ ескалювати "буде малий" (максимум XXL), джинси/лофери — свої
  // системи розмірів, не летерні.
  set_per_item_sizing: { over: {}, msgs: ['Артикул set1112', 'Весь комплект', '187 115'], check: (c) => c.isSetSizeCalc === true && /буде малий/i.test(c.setSizesText || '') && /оберіть самі/i.test(c.setSizesText || '') },
};
