...
(async () => {
    ...
    ok('B-V9b', 'зміна замовлення з кроку оплати: n_pay_collect → n_pay_back_cond → clear → n_order_intent; правило backToOrder у промпті; n_order_intent бачить orderChangeNote', next('n_pay_collect') === 'n_pay_back_cond' && branch('n_pay_back_cond', 'true') === 'n_pay_back_clear' && branch('n_pay_back_cond', 'false') === 'n_fs4' && next('n_pay_back_clear') === 'n_order_intent' && /backToOrder/.test(byId.n_pay_collect.data.systemPrompt) && /orderChangeNote/.test(byId.n_order_intent.data.systemPrompt));
