...
ok('K1b', 'гейт[true] → постачальник, [false] → одразу підтвердження (v10: без окремого алерту n_supplier_hold)', branch('n_supplier_pay_gate', 'true') === 'n_supplier_route' && branch('n_supplier_pay_gate', 'false') === 'n_confirm_prep' && !byId.n_supplier_hold);
ok('K1c', 'n_confirm_prep → n_confirm → n_fs5 → n_upsell2_wait', next('n_confirm_prep') === 'n_confirm' && next('n_confirm') === 'n_fs5' && next('n_fs5') === 'n_upsell2_wait');
ok('V10a', 'v10: усі notifyTg мають alertTitle/alertMain (єдиний формат); n_pay_notfound_admin прибрано', nodes.filter((n) => n.type === 'notifyTg').every((n) => n.data.alertTitle && typeof n.data.alertMain === 'string') && !byId.n_pay_notfound_admin && /createAlertMain/.test(byId.n_create.data.alertMain) && /orderUnitsText/.test(byId.n_create.data.alertDetails));
ok('V10b', 'v10: повторний прохід після чека → n_repeat_cond → (новий чек) один алерт → n_upsell2_wait, без n_create/n_confirm', branch('n_crm_order_cond', 'true') === 'n_repeat_cond' && branch('n_repeat_cond', 'false') === 'n_create' && branch('n_repeat_cond', 'true') === 'n_receipt_cond' && branch('n_receipt_cond', 'true') === 'n_receipt_alert' && branch('n_receipt_cond', 'false') === 'n_upsell2_wait' && next('n_receipt_alert') === 'n_upsell2_wait' && /repeatPass/.test(byId.n_crm_order.data.code) && /receiptNew/.test(byId.n_crm_order.data.code));
ok('V10c', 'v10: n_collect — 1–3 цифри = відділення, пояснення суми з даних; n_order_intent — формула ціни за кількість і «підганяння»; n_size відповідає на «в наявності?»', /1–3 цифри = відділення/.test(byId.n_collect.data.systemPrompt) && /НЕ вибачайся за «неправильну ціну»/.test(byId.n_collect.data.systemPrompt) && /3 шт = 799 \+ 449 = 1248/.test(byId.n_order_intent.data.systemPrompt) && /підганяє/.test(byId.n_order_intent.data.systemPrompt) && new RegExp(byId.n_size.data.waitAfterPresentationUnless, 'i').test('В наявності цей товар ще??'));
...
ok('V6a', 'відповідь на рекламу без синхронізованого ad_id: назва реклами → CRM ads → товар; keyword по назві; візія по картинці реклами; нова реклама реєструється в CRM', /via = 'ad_title'/.test(byId.n_lookup.data.code) && /__kwSource/.test(byId.n_lookup.data.code) && /__refImg/.test(byId.n_lookup.data.code) && /fetch\(base \+ '\/ads', \{ method: 'POST'/.test(byId.n_lookup.data.code));
// v12.4: між n_catalog_hint і n_unknown_msg — n_hint_pick_cond (клієнт назвав товар зі списку → n_prev_match_snapshot).
// 2026-09-XX: n_hint_pick_cond[false] тепер веде через n_catalog_hint_photo_cond/_mark (фото товару з підказки) — оновлено 2026-09-12.
ok('V5a', 'категорія без артикулу → n_catalog_hint → (n_hint_pick_cond) → фото-гілка → n_unknown_msg з підказкою; алерт лише без підказки', !!byId.n_catalog_hint && edges.every((e) => e.target !== 'n_unknown_msg' || ['n_catalog_hint', 'n_hint_pick_cond', 'n_catalog_hint_photo_cond', 'n_catalog_hint_photo_mark'].includes(e.source)) && next('n_catalog_hint') === 'n_hint_pick_cond' && edges.some((e) => e.source === 'n_hint_pick_cond' && e.sourceHandle === 'true' && e.target === 'n_prev_match_snapshot') && branch('n_hint_pick_cond', 'false') === 'n_catalog_hint_photo_cond' && edges.some((e) => e.source === 'n_catalog_hint_photo_cond' && e.sourceHandle === 'false' && e.target === 'n_unknown_msg') && /catalogHint/.test(byId.n_unknown_msg.data.systemPrompt) && /catalogHint/.test(byId.n_unknown_once_cond.data.condition));
...
ok('V4d', 'сигнал "товар не визначено" лише при явному сигналі товару; без тест-рестарту', byId.n_unknown_stop.data.testRestartAfter === false && (() => { try { const f = new Function('context', 'return ' + byId.n_unknown_once_cond.data.condition); return !f({ lastUserMessage: 'Добрий вечір)', hasProductSignal: false }) && !f({ lastUserMessage: 'яка ціна кофти?', hasProductSignal: false, catalogHint: 'Артикул A0187 — кофта' }) && f({ lastUserMessage: 'Артикул Z9999', hasProductSignal: true }); } catch (e) { return false; } })());
ok('V5b', 'n_unknown_msg: категорії, підказка, довідка у промпті; n_catalog_hint віддає catalogCategories', /catalogCategories/.test(byId.n_unknown_msg.data.systemPrompt) && /context\.shop\.faq/.test(byId.n_unknown_msg.data.systemPrompt) && /catalogCategories/.test(byId.n_catalog_hint.data.code));
// v7: база знань у CRM — профіль магазину з /knowledge/profile у context.shop, пошук по /knowledge/search (useCrmKb), вектор-база вимкнена
ok('V9a', 'n_shop_profile між n_route і наступним кроком; код читає /knowledge/profile', !!byId.n_shop_profile && next('n_route') === 'n_shop_profile' && ['n_signal_check', 'n_prev_match_snapshot'].includes(next('n_shop_profile')) && /knowledge\/profile/.test(byId.n_shop_profile.data.code) && /shop\.faq/.test(byId.n_shop_profile.data.code));
ok('V9b', 'у промптах нема {{env.SHOP_FAQ}}/{{env.ORDER_TERMS_LINE}} — лише context.shop.*', nodes.every((n) => n.type !== 'claude' || !/\{\{env\.(SHOP_FAQ|ORDER_TERMS_LINE)\}\}/.test(n.data.systemPrompt || '')) && /\{\{context\.shop\.terms\}\}/.test(byId.n_order_intent.data.systemPrompt));
ok('V9c', 'useCrmKb на діалогових нодах, useKb (вектор) вимкнено всюди', ['n_size', 'n_color', 'n_set_choice', 'n_order_intent'].every((id) => byId[id].data.useCrmKb === true) && nodes.every((n) => !n.data.useKb));
...
// v12.9: n_post_order_cond:true → n_post_order_once_cond (раз на 30 хв) → n_post_order_mark → n_post_order_msg; повтор → лише алерт менеджеру.
ok('V7c', 'n_lookup → ad_conflict_cond → post_order_cond → returning_check / once_cond → mark → msg', next('n_lookup') === 'n_ad_conflict_cond' && branch('n_ad_conflict_cond', 'false') === 'n_post_order_cond' && branch('n_ad_conflict_cond', 'true') === 'n_ad_conflict_admin' && next('n_ad_conflict_admin') === 'n_ad_conflict_mark' && next('n_ad_conflict_mark') === 'n_post_order_cond' && branch('n_post_order_cond', 'false') === 'n_returning_check' && branch('n_post_order_cond', 'true') === 'n_post_order_once_cond' && branch('n_post_order_once_cond', 'false') === 'n_post_order_mark' && next('n_post_order_mark') === 'n_post_order_msg' && branch('n_post_order_once_cond', 'true') === 'n_post_order_admin');
...
// 2026-09-12: точна фраза "ОБОВʼЯЗКОВО містить слово «Оформляємо»" видалена в 6e30e59 (реворкінг
// вступного повідомлення) — базовий сценарій без допродажу й досі завжди закінчується "Оформляємо
// замовлення?" (рядок 108), перевіряємо це замість застарілого дослівного формулювання.
ok('V3e', 'умови в підсумку (context.shop.terms з CRM) + «Оформляємо» — стандартне завершальне питання', /context\.shop\.terms/.test(byId.n_order_intent.data.systemPrompt) && /Оформляємо замовлення\?/.test(byId.n_order_intent.data.systemPrompt));
...
ok('V3g', 'askManager замість handoff на невідомі питання + довідка (context.shop.faq) у промптах; ключі SHOP_FAQ/ORDER_TERMS_LINE більше не пишуться', ['n_size', 'n_color', 'n_set_choice', 'n_order_intent'].every((id) => /askManager/.test(byId[id].data.systemPrompt) && /context\.shop\.faq/.test(byId[id].data.systemPrompt)) && !/поза твоїми даними \(гарантія, доставка за кордон, нестандартна оплата, знижки, претензія\) — НЕ вигадуй: поверни json_output \{"handoff"/.test(byId.n_color.data.systemPrompt) && !(r.keyUpdates || []).some((k) => k.key === 'SHOP_FAQ' || k.key === 'ORDER_TERMS_LINE'));
...
ok('V8b', 'n_avail_cond[true] → (фото допродажу один раз) → n_extra_resolve → n_order_intent', branch('n_avail_cond', 'true') === 'n_upsell_photo_cond' && branch('n_upsell_photo_cond', 'true') === 'n_upsell_photo' && branch('n_upsell_photo_cond', 'false') === 'n_extra_resolve' && next('n_upsell_photo') === 'n_upsell_photo_mark' && next('n_upsell_photo_mark') === 'n_extra_resolve' && next('n_extra_resolve') === 'n_extra_resolved_cond' && /ДОПРОДАЖ/.test(byId.n_order_intent.data.systemPrompt));
// v12.16 (п.5 складні кошики): усі входи в n_order_intent — через n_extra_resolve; після n_order_intent — n_extra_after_cond.
// 2026-09-12: n_extra_resolved_cond[false] тепер веде через n_avail_search (перевірка наявності по каталогу) — оновлено після цього хопу.
ok('V16a', 'n_extra_resolve → n_extra_resolved_cond → (n_avail_search) → n_order_intent (усі входи), n_extra_after_cond після', edges.filter((e) => e.target === 'n_order_intent').every((e) => e.source === 'n_avail_search') && next('n_extra_resolve') === 'n_extra_resolved_cond' && branch('n_extra_resolved_cond', 'false') === 'n_avail_search' && next('n_avail_search') === 'n_order_intent' && branch('n_extra_resolved_cond', 'true') === 'n_order_cond' && next('n_order_intent') === 'n_extra_after_cond' && branch('n_extra_after_cond', 'true') === 'n_extra_resolve' && branch('n_extra_after_cond', 'false') === 'n_order_cond' && /extraIntentResolved/.test(byId.n_extra_after_cond.data.condition) && /extraItemsText/.test(byId.n_order_intent.data.systemPrompt) && /"extras"/.test(byId.n_order_intent.data.systemPrompt));
ok('V16b', 'n_pay_amount рахує orderExtras/extrasSum; n_crm_order і brewdrop беруть orderExtras', /orderExtras/.test(byId.n_pay_amount.data.code) && /extrasSum/.test(byId.n_pay_amount.data.code) && /orderExtras/.test(byId.n_crm_order.data.code) && /orderExtras/.test(byId.n_supplier_order.data.code));
ok('V12a', 'v12: alsoWants у n_size/n_color/n_order_intent, silentOnExit, ігнор «ціна?» після презентації, фото клієнта в алерті', ['n_size', 'n_color'].every((id) => /"alsoWants"/.test(byId[id].data.systemPrompt) && byId[id].data.silentOnExit === true) && /КЛІЄНТ ТАКОЖ ХОЧЕ/.test(byId.n_order_intent.data.systemPrompt) && new RegExp(byId.n_size.data.ignoreRightAfterPresentationRe, 'i').test('Вітаю,ціна?') && !new RegExp(byId.n_size.data.ignoreRightAfterPresentationRe, 'i').test('Яка ціна доставки?') && byId.n_unknown_admin.data.alertPhoto === '{{context.lastUserImageUrl}}');
ok('V12c', 'v12: розмірна сітка на запит (wantsSizeChart у промптах), n_presented_recently_cond перед фото/карткою', ['n_size', 'n_color'].every((id) => /"wantsSizeChart"/.test(byId[id].data.systemPrompt)) && branch('n_have_product', 'true') === 'n_presented_recently_cond' && branch('n_presented_recently_cond', 'true') === 'n_fs1' && branch('n_presented_recently_cond', 'false') === 'n_has_photo' && /skipPresentation/.test(byId.n_lookup.data.code));
ok('V12b', 'v12: SIZE_FIRST_MSG_RE ловить «31 размера», «кофта М», «S»; n_lookup: звірка артикулу реклами, alsoWants, компоненти комплекту, keyword клієнта', ['есть джинсы 31 размера и кофта М?', 'Футболочку белую тоже ,S', 'В наявності?'].every((t) => new RegExp(byId.n_size.data.waitAfterPresentationUnless, 'i').test(t)) && !new RegExp(byId.n_size.data.waitAfterPresentationUnless, 'i').test('Як замовити кофту?') && /ad_caption_override/.test(byId.n_lookup.data.code) && /result\.alsoWants/.test(byId.n_lookup.data.code) && /ad_set_component/.test(byId.n_lookup.data.code) && /user_keyword/.test(byId.n_lookup.data.code) && /setComponentHint/.test(byId.n_catalog_hint_prep.data.code)); // 2026-09-12: маркер переїхав у n_catalog_hint_prep (httpRequest-рефакторинг)
...
(async () => {
    ...
    x = await runNode('n_calc', { testMode: true, product: { ...prodHW, sizes: ['S', 'M', 'L', 'XL', 'XXL'] }, sizeInput: { height: 100, weight: 180 } });
    ok('B-C2', 'n_calc "100 180" → міняє місцями (180/100 → XL), не ескалює', x.sizeOutOfRange === false && x.recommendedSize === 'XL', JSON.stringify(x.recommendedSize));
    x = await runNode('n_calc', { testMode: true, product: prodHW, sizeInput: { height: 185, weight: 120 } });
    ok('B-C16', 'n_calc 185/120 при S–L → ескалація «найбільший наявний L буде малий», а не L', x.sizeOutOfRange === true && /найбільший наявний/.test(x.sizeOorReason), x.sizeOorReason);
    ...
    ok('B-M8', 'v8.1: інший товар посеред чекауту — lockProduct на n_order_intent/n_pay_collect/n_collect, правило extraProducts у промпті, рядок у сповіщеннях і коментарі CRM', ['n_order_intent', 'n_pay_collect', 'n_collect'].every((id) => byId[id].data.lockProduct === true) && /"extraProducts"/.test(byId.n_order_intent.data.systemPrompt) && /extraProductsLine/.test(byId.n_create.data.message) && /extraProducts/.test(byId.n_crm_order.data.code) && /extraProductsLine/.test(byId.n_pay_amount.data.code));
    x = await runNode('n_pay_amount', { paymentInfo: { method: 'full' }, product: { price: 1799 }, orderIntent: { ready: 'yes', extraProducts: 'кофта A0187, чорна' } }, { user: {} });
    ok('B-M9', 'n_pay_amount: extraProducts → рядок для менеджера, сума лише за основний товар', /кофта A0187/.test(x.extraProductsLine) && x.payAmount === 1799, x.extraProductsLine);
    ok('B-M10', 'n_lookup: акція за кількість з опису «(2 шт: 2199 ₴)», якщо bulkPricing у CRM порожнє', /__promoRe/.test(byId.n_lookup.data.code) && (() => { const re = /(\d{1,2})\s*шт\.?\s*[:—–-]\s*(\d[\d\s]{2,7})\s*(?:₴|грн)/gi; const m = re.exec('💵 Акційна ціна кофти: 1279 ₴ (2 шт: 2199 ₴)'); return !!m && m[1] === '2' && m[2].trim() === '2199'; })());
    // v9 (тести Олексія 2026-09-06)
    ok('B-V9a', 'ibanoplata без посилання → n_iban_ok_cond[false] → реквізити вручну (n_req_fallback_msg → n_req_iban_l), без паузи', next('n_iban_invoice') === 'n_iban_ok_cond' && branch('n_iban_ok_cond', 'true') === 'n_requisites' && branch('n_iban_ok_cond', 'false') === 'n_req_fallback_msg' && next('n_req_fallback_msg') === 'n_req_iban_l' && (() => { const f = new Function('context', 'return ' + byId.n_iban_ok_cond.data.condition); return f({ ibanPayUrl: 'https://ibanoplata.com/iban-qr/abc' }) === true && f({ ibanPayUrl: '' }) === false && f({}) === false; })());
    ok('B-V9b', 'зміна замовлення з кроку оплати: n_pay_collect → n_pay_back_cond → clear → n_order_intent; правило backToOrder у промпті; n_order_intent бачить orderChangeNote', next('n_pay_collect') === 'n_pay_back_cond' && branch('n_pay_back_cond', 'true') === 'n_pay_back_clear' && branch('n_pay_back_cond', 'false') === 'n_fs4' && next('n_pay_back_clear') === 'n_upsell_photo_cond' && /backToOrder/.test(byId.n_pay_collect.data.systemPrompt) && /orderChangeNote/.test(byId.n_order_intent.data.systemPrompt));
    x = await runNode('n_pay_back_clear', { paymentInfo: { backToOrder: true, note: 'додати білу футболку' }, orderIntent: { ready: 'yes' } });
    ok('B-V9c', 'n_pay_back_clear: примітка передається, paymentInfo/orderIntent скинуто', x.orderChangeNote === 'додати білу футболку' && x.paymentInfo === null && x.orderIntent === null);
    ok('B-V9d', 'n_order_intent уточнює «так» при запропонованому допродажі; n_collect: «нема посилання» → wantsManualReq', /ВИНЯТОК: якщо в тому самому питанні ти пропонувала ДОПРОДАЖ/.test(byId.n_order_intent.data.systemPrompt) && /посилання на оплату немає/.test(byId.n_collect.data.systemPrompt));
    // v10: ціна за кількість наборами, розмір за вагою з поправкою на зріст, наявність по кольору+розміру
    x = await runNode('n_pay_amount', { paymentInfo: { method: 'full' }, product: { price: 1799, upsellItems: [{ id: 'u', name: 'Футболка', price: 449, qtyPrices: { '2': 799 } }] }, orderIntent: { ready: 'yes', addUpsell: true, upsellQty: 3 } }, { user: {} });
    ok('B-T1', 'допродаж 3 шт при «2 — 799» → 799 + 449 = 1248 (не 799 і не 1347)', x.upsellSum === 1248 && x.payAmount === 1799 + 1248 && /Футболка ×3 \(1248 грн\)/.test(x.upsellLine), JSON.stringify([x.upsellSum, x.upsellLine]));
    x = await runNode('n_pay_amount', { paymentInfo: { method: 'full' }, product: { price: 1279, qtyPrices: { '2': 2199 } }, orderIntent: { ready: 'yes', qty: 3 } }, { user: {} });
    ok('B-T2', 'товар 3 шт при «2 — 2199» → 2199 + 1279 = 3478', x.payAmount === 3478 && x.orderQty === 3, String(x.payAmount));
    x = await runNode('n_avail', { product: { price: 1279, qtyPrices: { '2': 2199 }, sizes: ['M'], offers: [] }, colorChoice: { color: 'Чорний', qty: 3 }, recommendedSize: 'M' });
    ok('B-T3', 'n_avail: 3 шт → сума наборами 3478', x.orderUnitsTotal === 3478, String(x.orderUnitsTotal));
    x = await runNode('n_calc', { testMode: true, product: prodHW, sizeInput: { height: 190, weight: 63 } });
    ...
    ok('B-A8', 'n_avail: «доступно завжди» → наявний незалежно від кількостей', x.available === true);
    x = await runNode('n_avail', { product: { stockTracked: true, offers: [{ properties: [{ name: 'Колір', value: 'Чорний' }, { name: 'Розмір', value: 'M' }], quantity: 2 }, { properties: [{ name: 'Колір', value: 'Чорний' }, { name: 'Розмір', value: 'L' }], quantity: 0 }] }, colorChoice: { color: 'Чорний' }, recommendedSize: 'L' });
    ok('B-A9', 'n_avail: кількості по розмірах — Чорний L = 0 → нема (хоча Чорний M є)', x.available === false && x.unavailableColors[0] === 'Чорний', JSON.stringify(x));
    x = await runNode('n_avail', { product: { stockTracked: true, offers: [{ properties: [{ name: 'Колір', value: 'Чорний' }, { name: 'Розмір', value: 'M' }], quantity: 2 }] }, colorChoice: { color: 'Чорний' }, recommendedSize: 'M' });
    ok('B-A10', 'n_avail: кількості по розмірах — Чорний M = 2 → є', x.available === true);
    x = await runNode('n_order_prefill', { crmOrderId: 'old-1', orderRef: 'GOVOLD', payStatus: 'confirmed', payAmount: 1728, product: { customerName: 'Кофта' }, orderData: { fullName: 'Іван', phone: '0971', city: 'Львів', branch: '14' }, orderIntent: { ready: 'yes' } });
    ok('B-V9e', 'n_order_prefill: повторне замовлення в сесії → попереднє в prevOrders, orderRef/crmOrderId/оплата скинуті, адреса лишається і не перепитується', x.prevOrders && x.prevOrders[0].crmOrderId === 'old-1' && x.crmOrderId === null && x.orderRef === null && x.payStatus === null && x.orderData === undefined && x.recalledDeliveryReady === true, JSON.stringify(x));
    ok('B-V9f', 'n_order_intent: примітка зміни з кроку оплати обовʼязково потрапляє в підсумок і json (addUpsell/upsellQty)', /клієнт уже погодився оформити і повернувся сюди ЛИШЕ щоб змінити/.test(byId.n_order_intent.data.systemPrompt) && /"addUpsell":true,"upsellQty":<n>/.test(byId.n_order_intent.data.systemPrompt));
