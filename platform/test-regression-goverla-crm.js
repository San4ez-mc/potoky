...
ok('V5a', 'категорія без артикулу → n_catalog_hint → n_unknown_msg з підказкою; алерт лише без підказки', !!byId.n_catalog_hint && edges.every((e) => e.target !== 'n_unknown_msg' || e.source === 'n_catalog_hint') && next('n_catalog_hint') === 'n_unknown_msg' && /catalogHint/.test(byId.n_unknown_msg.data.systemPrompt) && /catalogHint/.test(byId.n_unknown_once_cond.data.condition));
