...
ok('V5a', 'категорія без артикулу → n_catalog_hint → (n_hint_pick_cond) → n_unknown_msg з підказкою; алерт лише без підказки', !!byId.n_catalog_hint && edges.every((e) => e.target !== 'n_unknown_msg' || e.source === 'n_catalog_hint' || e.source === 'n_hint_pick_cond') && next('n_catalog_hint') === 'n_hint_pick_cond' && edges.some((e) => e.source === 'n_hint_pick_cond' && e.sourceHandle === 'true' && e.target === 'n_prev_match_snapshot') && edges.some((e) => e.source === 'n_hint_pick_cond' && e.sourceHandle === 'false' && e.target === 'n_unknown_msg') && /catalogHint/.test(byId.n_unknown_msg.data.systemPrompt) && /catalogHint/.test(byId.n_unknown_once_cond.data.condition));
...
ok('V3e', 'умови в підсумку (context.shop.terms з CRM) + слово «Оформляємо» обовʼязкове', /context\.shop\.terms/.test(byId.n_order_intent.data.systemPrompt) && /ОБОВʼЯЗКОВО містить слово «Оформляємо»/.test(byId.n_order_intent.data.systemPrompt));
