...
ok('V8b', 'n_avail_cond[true] → (фото допродажу один раз) → n_extra_resolve → n_order_intent', branch('n_avail_cond', 'true') === 'n_upsell_photo_cond' && branch('n_upsell_photo_cond', 'true') === 'n_upsell_photo' && branch('n_upsell_photo_cond', 'false') === 'n_extra_resolve' && next('n_upsell_photo') === 'n_upsell_photo_mark' && next('n_upsell_photo_mark') === 'n_extra_resolve' && next('n_extra_resolve') === 'n_order_intent' && /ДОПРОДАЖ/.test(byId.n_order_intent.data.systemPrompt));
// v12.16 (п.5 складні кошики): усі входи в n_order_intent — через n_extra_resolve; після n_order_intent — n_extra_after_cond.
ok('V16a', 'n_extra_resolve перед n_order_intent (усі входи), n_extra_after_cond після', edges.filter((e) => e.target === 'n_order_intent').every((e) => e.source === 'n_extra_resolve') && next('n_order_intent') === 'n_extra_after_cond' && branch('n_extra_after_cond', 'true') === 'n_extra_resolve' && branch('n_extra_after_cond', 'false') === 'n_order_cond' && /extraItemsText/.test(byId.n_order_intent.data.systemPrompt) && /"extras"/.test(byId.n_order_intent.data.systemPrompt));
