...
(async () => {
    ...
    ok('B-V9d', 'n_supplier_hold: посилання на Instagram клієнта і позиції; n_order_intent уточнює «так» при запропонованому допродажі; n_collect: «нема посилання» → wantsManualReq', /instagram\.com\/\{\{context\.igUsername\}\}/.test(byId.n_supplier_hold.data.message) && /orderUnitsText/.test(byId.n_supplier_hold.data.message) && /ВИНЯТОК: якщо в тому самому питанні ти пропонувала ДОПРОДАЖ/.test(byId.n_order_intent.data.systemPrompt) && /посилання на оплату немає/.test(byId.n_collect.data.systemPrompt));
