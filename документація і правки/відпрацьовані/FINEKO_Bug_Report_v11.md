...
**Статус:** ⚙️ Частково виконано (готово технічно, потрібне заповнення значень)  
**Пріоритет:** Середній  
**Bot ID:** `e7dd3dd2-3a5b-438a-bdd3-beda815f7681`

**Що вже зроблено технічно:**
- Додані системні ключі в Налаштування → Ключі: `ADMIN_TELEGRAM_ID`, `COURSE_PRICE`, `COURSE_PRICE_INT`.
- Нода `notifyAdmin` читає `ADMIN_TELEGRAM_ID` і `COURSE_PRICE` саме із системних ключів.
...
**Статус:** ✅ Виправлено (інфраструктурно)  
**Пріоритет:** Високий  
**Bot ID:** `e7dd3dd2-3a5b-438a-bdd3-beda815f7681`

**Що реалізовано в платформі:**
- Нода `wait` з персистентним таймером у `flowRuntime` (переживає рестарти API, бо стан зберігається в БД сесії).
- Нода `condition` з true/false гілками.
- Нода `sendDocument` для відправки/вкладення документів (через `fileType` або `fileVar`).
- Нода `wait_payment` з гілками `paid` / `unpaid` і timeout.
- Нода `notifyAdmin` для сповіщення адміна + опційне повідомлення студенту.
